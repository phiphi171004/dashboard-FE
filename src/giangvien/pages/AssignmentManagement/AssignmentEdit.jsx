...
const AssignmentEdit = () => {
  ...
  const loadAssignmentData = async () => {
    try {
      ...
      const assignment = mockAssignmentData.assignments.find(a => a.id === parseInt(id));
      if (assignment) {
        setFormData({
          ...
          showScoreToStudents: assignment.showScoreToStudents !== false,
          requireFiles: assignment.requireFiles !== false,
          allowedFileTypes: assignment.allowedFileTypes || ['pdf', 'doc', 'docx', 'txt'],
          maxFileSize: assignment.maxFileSize || 10
        });
        
        if (assignment.files) {
          setAttachedFiles(assignment.files);
        }
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Here you would make an actual API call to update the assignment
      console.log('Updating assignment:', { id, ...formData, attachedFiles });
